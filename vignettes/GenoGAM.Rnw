...
%% Remove once BiocStyle works
%% \newcommand{\Robject}{\texttt}
%% \newcommand{\Rclass}{\texttt}
%% \newcommand{\Rfunction}{\texttt}
...
Finally, the last line of code calls the function \Rfunction{subset()} to restrict the \Rclass{GenoGAMDataset} to the positions of interest. This line is necsessary for running this small example but would not be present in a standard genome-wide run of \genogam{}.
...
We test for each smooth and at each position $x$ the null hypothesis that it values 0 by a call to \Rfunction{computeSignificance()}. False discovery rate can bve computed using the Benjamini-Hochberg procedure with the R function \Rfunction{p.adjust()}: 
...
We thank Alexander Engelhardt, Herv\'e Pag\`es, and Martin Morgan for input in the developement of \genogam{}. 
