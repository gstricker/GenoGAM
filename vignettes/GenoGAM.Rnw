...
The GenoGAM specific plot function will use make use of the ggplot2 framework if available or fall back to base R otherwise.
...
plot(fit, ggd, ranges = gr, scale = FALSE)
@
...
Other functionalities demonstrated in the manuscript (peak calling and testing, methylation data, see BioRxiv: \url{http://dx.doi.org/10.1101/047464}) will be gradually integrated into the package. Interested users should check the developer version of \Biocpkg{GenoGAM} for updates.
